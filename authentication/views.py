...
from authentication.bussiness import send_verify_code, register_by_email, login_by_email, reset_pwd_by_email, \
    get_user_info, update_user_info, get_user_list, login_by_email_code
...
class ResetPasswordByEmailView(APIView):
    ...
    def post(self, request):
        try:
            ...
            verify_code = request.data.get('verify_code', None)
            password = request.data.get('password', None)
            code, resp = reset_pwd_by_email(request, email, password, verify_code)
            if code == RespCode.CREATED.value:
                return reformat_resp(code, resp, 'Succeed')
            else:
                return reformat_resp(code, {}, resp)
...
class UserListView(APIView):
    ...
    def get(self, request):
        try:
            code, resp = get_user_list(request)
            if code == RespCode.OK.value:
                return reformat_resp(code, resp, 'Succeed')
            else:
                return reformat_resp(code, {}, resp)
